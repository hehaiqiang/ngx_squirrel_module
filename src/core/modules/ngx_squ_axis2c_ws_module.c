...
static int ngx_squ_webservice_parse(squ_State *l);
static void ngx_squ_webservice_parse_children(squ_State *l, axutil_env_t *env,
    axiom_node_t *parent, axiom_element_t *parent_elem);

static int ngx_squ_webservice_serialize(squ_State *l);
static void ngx_squ_webservice_serialize_tables(squ_State *l, axutil_env_t *env,
    axiom_node_t *parent);
static void ngx_squ_webservice_serialize_table(squ_State *l, axutil_env_t *env,
    axiom_node_t *parent, char *name, int index);

static ngx_int_t ngx_squ_webservice_module_init(ngx_cycle_t *cycle);


static ngx_squ_const_t  ngx_squ_webservice_consts[] = {
    { NULL, 0 }
};


static squL_Reg  ngx_squ_webservice_methods[] = {
    { "parse", ngx_squ_webservice_parse },
    { "serialize", ngx_squ_webservice_serialize },
    { NULL, NULL }
};


ngx_module_t  ngx_squ_webservice_module = {
    NGX_MODULE_V1,
    NULL,                                  /* module context */
    NULL,                                  /* module directives */
    NGX_CORE_MODULE,                       /* module type */
    NULL,                                  /* init master */
    ngx_squ_webservice_module_init,        /* init module */
    NULL,                                  /* init process */
    NULL,                                  /* init thread */
    NULL,                                  /* exit thread */
    NULL,                                  /* exit process */
...
static int
ngx_squ_webservice_parse(squ_State *l)
{
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_CORE, thr->log, 0, "squ webservice parse");

    soap.data = (u_char *) squL_checklstring(l, 1, &soap.len);
    ...
    if (header != NULL) {
        ...
        ngx_squ_webservice_parse_children(l, env, node, elem);

        squ_setfield(l, -2, "header");
    }
...
static void
ngx_squ_webservice_parse_children(squ_State *l, axutil_env_t *env,
    axiom_node_t *parent, axiom_element_t *parent_elem)
{
    int                              n;
    ...
    do {
        ...
        if (attrs != NULL) {
            ...
            do {
                ...
                squ_pushstring(l, axiom_attribute_get_value(attr, env));
                squ_setfield(l, -2, axiom_attribute_get_localname(attr, env));
...
static int
ngx_squ_webservice_serialize(squ_State *l)
{
    int                     top;
    char                   *uri, *prefix;
    axiom_node_t           *node;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_CORE, thr->log, 0, "squ webservice serialize");
    ...
    if (!squ_isnil(l, -1)) {
        ...
        ngx_squ_webservice_serialize_tables(l, env, node);
    }
...
static void
ngx_squ_webservice_serialize_tables(squ_State *l, axutil_env_t *env,
    axiom_node_t *parent)
{
    int                type, n, i, index;
    char              *name;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_CORE, thr->log, 0,
                   "squ webservice serialize tables");
    ...
    while (squ_next(l, -2)) {
        ...
        ngx_squ_webservice_serialize_table(l, env, parent, name, index);
...
static void
ngx_squ_webservice_serialize_table(squ_State *l, axutil_env_t *env,
    axiom_node_t *parent, char *name, int index)
{
    int                 top;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_CORE, thr->log, 0,
                   "squ webservice serialize table");
...
static ngx_int_t
ngx_squ_webservice_module_init(ngx_cycle_t *cycle)
{
    int              n;
    ngx_squ_conf_t  *scf;

    ngx_log_debug0(NGX_LOG_DEBUG_CORE, cycle->log, 0,
                   "squ webservice module init");

    scf = (ngx_squ_conf_t *) ngx_get_conf(cycle->conf_ctx, ngx_squ_module);

    squ_getglobal(scf->l, NGX_SQU_TABLE);

    n = sizeof(ngx_squ_webservice_consts) / sizeof(ngx_squ_const_t) - 1;
    n += sizeof(ngx_squ_webservice_methods) / sizeof(squL_Reg) - 1;

    squ_createtable(scf->l, 0, n);

    for (n = 0; ngx_squ_webservice_consts[n].name != NULL; n++) {
        squ_pushinteger(scf->l, ngx_squ_webservice_consts[n].value);
        squ_setfield(scf->l, -2, ngx_squ_webservice_consts[n].name);
    }

    for (n = 0; ngx_squ_webservice_methods[n].name != NULL; n++) {
        squ_pushcfunction(scf->l, ngx_squ_webservice_methods[n].func);
        squ_setfield(scf->l, -2, ngx_squ_webservice_methods[n].name);
    }

    squ_setfield(scf->l, -2, "webservice");
